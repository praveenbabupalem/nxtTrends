...
class AllProductsSection extends Component {
  ...
  applySearch = inputValue => {
    this.setState({titleSearch: inputValue}, this.getProducts)
  }
  ...
  getProducts = async () => {
    ...
    const {category, rating, titleSearch, activeOptionId} = this.state
    ...
    if (response.status === 401) {
      this.setState({isFetchSuccess: false, isLoading: false})
    }
  }
  ...
  renderProductsList = () => {
    const {productsList, activeOptionId, isFetchSuccess} = this.state
    ...
    if (productsList.length === 0) {
      return (
        <div className="all-products-container">
          ...
          <p>We couldn&apos;t find any products. Try other filter</p>
        </div>
      )
    }
